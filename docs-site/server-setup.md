...
The `@domos/server` package provides the server integration for the OwlLayer AI Runtime. It orchestrates active AI sessions, handles LLM translation adapters, and executes safety validations.
...
The OwlLayer AI Runtime supports both a classic raw string prompt and a structured `SystemPromptConfig` object. The structured prompt compiles automatically to structure-enforced formats, ensuring predictable model guidance.
