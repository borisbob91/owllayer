...
Dans l'architecture actuelle de DomOS, les outils (tools) déclarés via les hooks/composables (ex: `useAgent`) sont étroitement couplés au cycle de vie des composants qui les enregistrent.
...
3. `HomePage` se démonte : DomOSClient reçoit l'ordre de désenregistrer `navigate_to`.
...
Nous avons créé un pont direct entre l'initialisation du framework (le Provider / Plugin) et le `DomOSCore`. Tous les outils définis dans la branche "globale" reçoivent un `componentId` interne de sécurité (`global-provider`) qui empêche le nettoyage lors du changement de composant React/Vue/Svelte.

---

## 🏗️ Implémentation & Patch par Framework

### 1. React (`@domos/react`) - ✅ Implémenté

Ajout de la propriété `globalTools` directement dans le `DomOSProvider`. Ces outils sont enregistrés juste après la création du client WebSocket.
...
export default function App() {
  return (
    ...
    <DomOSProvider apiKey="..." endpoint="..." globalTools={GLOBAL_TOOLS}>
      <Router>
        <Routes>...</Routes>
      </Router>
    </DomOSProvider>
...
### 2. Vue (`@domos/vue`) - ✅ Implémenté

Dans Vue, l'initialisation de DomOS se fait typiquement via un plugin (ex: `DomOSPlugin`).

**Évolution :**
Modification de `DomOSPluginOptions` et de la fonction d'installation pour accepter `globalTools`.
...
app.use(DomOSPlugin, {
  ...
  endpoint: import.meta.env.VITE_DOMOS_ENDPOINT,
...
### 3. Svelte (`@domos/svelte`) - ✅ Implémenté

Dans Svelte, l'initialisation de DomOS se fait via la fonction `initDomOS` appelée au niveau Racine/Layout.

**Évolution :**
Création de `DomOSInitOptions` pour remplacer `DomOSClientOptions` et prise en charge de `globalTools`.
...
- **Protection Core** : Côté `@domos/core`, on utilise un `componentId` réservé (`'global-provider'`) qui est ignoré par les fonctions de type `unregisterToolsByComponent()`.
