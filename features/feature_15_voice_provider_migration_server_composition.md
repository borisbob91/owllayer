# Feature 15 — Sprint 2 : Migration des providers voice/speech hors de `@domos/server` vers `adapter-google` / `adapter-openai`, réduction de `server` à la composition runtime
...
Ce sprint suppose que les contrats et bases abstraites voice/speech ont déjà été déplacés vers `@domos/core`. Il s'agit maintenant de déplacer les implémentations concrètes au bon endroit : dans les packages adapters provider.
...
| --- | --- | --- |
| `VOICE-COMP-001` | `packages/server/src/standalone/adapters/factory.ts` importe encore `../../speech/providers/*.js` pour Google/OpenAI | Refus du sprint tant que la factory n'importe pas les adapters speech depuis `@domos/adapter-google` et `@domos/adapter-openai` |
...
| `VOICE-COMP-003` | Un provider speech migré recopie de l'infrastructure audio générique au lieu d'utiliser `@domos/audio` | Refus du sprint tant que la logique est recentralisée correctement |
| `VOICE-COMP-004` | Les packages adapters speech nouveaux réintroduisent une dépendance structurelle à `@domos/server` | Refus du sprint tant que le provider compile contre `@domos/core` et `@domos/audio` uniquement |

---

## Découpage du sprint

### Bloc 1 — Migrer les providers speech Google vers `@domos/adapter-google`
...
- `packages/server/src/standalone/adapters/factory.ts` les charge via `@domos/adapter-google`.
...
### Bloc 2 — Migrer les providers speech OpenAI vers `@domos/adapter-openai`
...
- `packages/server/src/standalone/adapters/factory.ts` les charge via `@domos/adapter-openai`.
...
| `packages/adapter-google/package.json` | Dépendances taillées pour LLM/live | Dépendances alignées sur speech si besoin de `@domos/audio` | Supporter proprement les providers speech migrés |
...
| `packages/adapter-openai/package.json` | Dépendances taillées pour LLM/live | Dépendances alignées sur speech si besoin de `@domos/audio` | Supporter proprement les providers speech migrés |

### `packages/server`

| Fichier | AVANT | APRÈS | POURQUOI |
| --- | --- | --- | --- |
| `packages/server/src/standalone/adapters/factory.ts` | Importe speech depuis `../../speech/providers/*.js` | Importe les providers speech migrés depuis `@domos/adapter-google` et `@domos/adapter-openai` | Faire du server un composeur runtime seulement |
...
- Nettoyer les exports publics `@domos/server` pour refléter la nouvelle architecture.
- Réutiliser `@domos/audio` pour toute logique audio générique requise par les providers migrés.
...
- `packages/adapter-google` exporte `GoogleSTT` et `GoogleTTS` et compile avec `@domos/core` comme contrat partagé.
- `packages/adapter-openai` exporte `WhisperSTT` et `OpenAITTS` et compile avec `@domos/core` comme contrat partagé.
- `packages/server/src/index.ts` n'exporte plus les providers Google/OpenAI migrés.
- `pnpm --filter @domos/server build`, `pnpm --filter @domos/adapter-google build` et `pnpm --filter @domos/adapter-openai build` passent après la bascule.
...
| Jour 2 | Créer la structure speech dans `packages/adapter-google` et porter `GoogleSTT` | `GoogleSTT` existe dans `adapter-google` et compile contre `@domos/core` | Vérifier qu'aucun import résiduel vers `server` ne subsiste |
| Jour 3 | Porter `GoogleTTS` dans `packages/adapter-google` et mettre à jour `src/index.ts` | `adapter-google` expose désormais LLM/live/STT/TTS Google | Vérifier l'usage de `@domos/audio` pour les helpers génériques au lieu d'une recopie locale |
| Jour 4 | Construire et tester `@domos/adapter-google`, puis corriger les écarts d'API | Package Google speech validé | Bloquer si l'API publique diverge des contrats définis dans `@domos/core` |
| Jour 5 | Créer la structure speech dans `packages/adapter-openai` et porter `WhisperSTT` | `WhisperSTT` existe dans `adapter-openai` | Vérifier les usages OpenAI et la compatibilité des types importés depuis `@domos/core` |
| Jour 6 | Porter `OpenAITTS` dans `packages/adapter-openai` et mettre à jour `src/index.ts` | `adapter-openai` expose désormais LLM/live/STT/TTS OpenAI | Vérifier l'absence de duplication audio générique et l'absence de dépendance `@domos/server` |
...
- Hypothèse technique : aucune dépendance npm nouvelle n'est requise si les providers speech migrés réutilisent `@domos/audio` plutôt que de réimplémenter des helpers audio.
