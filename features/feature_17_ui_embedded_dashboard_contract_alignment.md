# Feature 17 — Sprint 1 : Alignement contractuel du dashboard embarqué `@owllayer/ui`
...
Rendre le dashboard embarqué de `@owllayer/ui` autonome et cohérent avec les contrats canoniques du monorepo, sans aucune dépendance fonctionnelle ou conceptuelle à `apps/dashboard`.
...
- `packages/ui/src/dashboard/api.ts` redéfinit localement `SystemPromptConfig` au lieu d'utiliser le contrat canonique exporté par `@owllayer/core`.
...
- Reroute le contrat `SystemPromptConfig` du dashboard embarqué vers `@owllayer/core`.
...
- `packages/ui` consomme les contrats partagés depuis `@owllayer/core`, jamais via une copie locale si le contrat existe déjà.
...
| --- | --- | --- | --- |
| `packages/ui/src/dashboard/api.ts` | Redéfinit `SystemPromptConfig` localement | Réutilise et ré-exporte le contrat canonique de `@owllayer/core` | Éviter une dérive de contrat |
...
- `packages/ui` ne redéfinit plus `SystemPromptConfig` si `@owllayer/core` l'exporte déjà.
...
- Basculer `packages/ui/src/dashboard/api.ts` sur les types exportés par `@owllayer/core`.
...
- Builder `@owllayer/ui`.
- Builder `@owllayer/server` pour valider le branchement du dashboard embarqué servi par le serveur.
