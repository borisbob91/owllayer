...
- Phase B `bridges DevTools` : faire passer ce flux à `@owllayer/ui/devtools` depuis tous les SDKs front
- Phase C `ui` : faire consommer ce flux par `@owllayer/ui/devtools` au lieu de dépendre principalement du polling et d'états reconstruits
...
- callbacks traduits à la main dans `OwlLayerProvider`, `OwlLayerPlugin`, les stores Svelte et `BrowserOwlLayer`
...
3. React, Vue, Svelte et Browser consomment le contrat d'événements via `@owllayer/core`.
...
Dans [packages/react/src/provider/OwlLayerProvider.tsx](packages/react/src/provider/OwlLayerProvider.tsx), le provider reconstruit déjà un modèle d'état à partir des callbacks de `OwlLayerClient`.

Dans [packages/vue/src/plugin/OwlLayerPlugin.ts](packages/vue/src/plugin/OwlLayerPlugin.ts), le plugin Vue reconstruit lui aussi état, audio output et approvals à partir des callbacks.

Dans [packages/svelte/src/stores/owllayer.store.ts](packages/svelte/src/stores/owllayer.store.ts), les stores rejouent ces mêmes callbacks vers des stores et listeners locaux.

Dans [packages/browser/src/runtime/BrowserOwlLayer.ts](packages/browser/src/runtime/BrowserOwlLayer.ts), le runtime browser remappe encore les callbacks de `OwlLayerClient` vers son propre état agent, ses overlays et son widget.

Dans [packages/react/src/plugins/useDevTools.ts](packages/react/src/plugins/useDevTools.ts), le bridge vers `@owllayer/ui/devtools` injecte :
...
- [packages/browser/src/runtime/BrowserOwlLayer.ts](packages/browser/src/runtime/BrowserOwlLayer.ts)
...
- `BrowserOwlLayer` possède une notion locale de fin réelle de playback, mais elle n'est pas encore standardisée comme surface DX commune à React, Vue, Svelte et Browser
...
- `subscribeEvent`
- `subscribeAnyEvent`
- éventuellement un hook/composable `useOwlLayerEvent(type, handler)` ou équivalent
- éventuellement une surface `useOwlLayerEventLog()` ou équivalent en dev
...
`OwlLayerProvider`, `OwlLayerPlugin`, les stores Svelte et `BrowserOwlLayer` cessent d'être les seules couches à connaître le détail des callbacks historiques.

### Phase B — Bridges DevTools

Les bridges React/Vue/Svelte/Browser vers `@owllayer/ui/devtools` transmettent désormais :
...
`@owllayer/ui/devtools` reçoit un contrat enrichi du type :
...
|---|---|---|---|
| `packages/react/src/provider/OwlLayerContext.ts` | contexte centré getters + actions | ajout des surfaces d'abonnement events | DX hooks/dev plus simple |
| `packages/react/src/provider/OwlLayerProvider.tsx` | callbacks de `OwlLayerClient` traduits localement | exposition du flux canonique vers le contexte | réduire la duplication locale |
...
| `packages/react/src/hooks/useOwlLayerEvent.ts` | absent | hook dédié | DX client simple |

### Phase A bis — Vue / Svelte / Browser

| Fichier | AVANT | APRÈS | POURQUOI |
|---|---|---|---|
| `packages/vue/src/plugin/OwlLayerPlugin.ts` | callbacks de `OwlLayerClient` traduits localement | exposition du flux canonique vers le plugin et les composables | réduire la duplication locale |
| `packages/vue/src/composables/useDevTools.ts` | bridge getters seulement | bridge getters + events | DevTools live plus fidèle |
| `packages/svelte/src/stores/owllayer.store.ts` | stores alimentés par callbacks locaux | exposition du flux canonique vers les stores et listeners | DX Svelte plus uniforme |
...
| `packages/browser/src/runtime/BrowserOwlLayer.ts` | runtime browser fondé sur callbacks nommés | exposition du flux canonique pour le runtime et le DevTools | DX browser uniforme |
...
- relever les usages actuels des callbacks de `OwlLayerClient` dans React, Vue, Svelte et Browser
...
- enrichir `OwlLayerContext`
- brancher le flux canonique dans `OwlLayerProvider`
...
- enrichir `OwlLayerPlugin`
- enrichir les stores Svelte
- enrichir `BrowserOwlLayer`
...
2. leurs bridges DevTools transmettent ce flux à `@owllayer/ui/devtools`
...
5. `pnpm --filter @owllayer/react build` passe
6. `pnpm --filter @owllayer/vue build` passe
7. `pnpm --filter @owllayer/svelte build` passe
8. `pnpm --filter @owllayer/browser build` passe
9. `pnpm --filter @owllayer/ui build` passe
