...
import { ADTPTransport } from '../src/transport/adtp.transport.js';
...
describe('DomOSServer lifecycle and hosting', () => {
  it('wires line HTTP routes when configureLines is called after construction', () => {
    const server = new DomOSServer({ llm: createLLM() });
  ...
  it('does not write a DomOS 404 over routes owned by an embedded HTTP server', async () => {
    ...
    const transport = new ADTPTransport(
      {
        server: httpServer,
        path: '/domos',
        httpHandler: () => false,
  ...
  it('serves embedded HTTP handlers in WebRTC mode', async () => {
    ...
    const transport = new WebRTCTransport(
      {
        server: httpServer,
        signalingPath: '/domos/rtc',
        httpHandler: (req, res) => {
          if (req.url === '/domos-ui/status') {
    ...
    try {
      const port = (httpServer.address() as AddressInfo).port;
      await expect(request(port, '/domos-ui/status')).resolves.toEqual({
        status: 200,
