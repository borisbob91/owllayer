# Sprint: `DomOSTool` + `DomOSToolBtn` — @domos/svelte
...
L'API Svelte actions supporte `update(newParams)` — quand les paramètres de `use:agentTool` changent, Svelte appelle automatiquement `update`. Le `DomOSTool.svelte` ne gère pas manuellement la re-registration : il suffit que `fullDescription` (snippet) soit réactif via `$derived`.
...
| Fichier | Action |
|---|---|
| `src/components/tool/DomOSTool.svelte` | CRÉER |
| `src/components/tool/DomOSToolBtn.svelte` | CRÉER |
...
## Phase 1 — `DomOSTool.svelte`

### `src/components/tool/DomOSTool.svelte`
...
<script lang="ts">
  ...
  // Validation runtime
  if (action && handler) {
    throw new Error(`DomOSTool "${name}": utilisez action OU handler, pas les deux.`);
  }
  if (!action && !handler) {
    throw new Error(`DomOSTool "${name}": action ou handler requis.`);
  }
...
## Phase 2 — `DomOSToolBtn.svelte`

### `src/components/tool/DomOSToolBtn.svelte`
...
```bash
pnpm --filter @domos/svelte build       # exit 0, pas d'erreurs TS/Svelte
pnpm --filter demo-svelte dev           # DomOSToolBtn "Vider" cliquable agent + humain
```

Vérifier dans devtools : le `<span style="display: contents">` dans `DomOSTool` n'ajoute aucun layout visible.
...
<!-- ✅ DomOSTool + action="click" — lien SvelteKit -->
<DomOSTool
  name="start_checkout"
