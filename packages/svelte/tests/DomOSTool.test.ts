...
import { domosClient } from '../src/stores/domos.store.js';
import { agentTool } from '../src/actions/useAgentTool.js';
import DomOSTool from '../src/components/tool/DomOSTool.svelte';
import DomOSToolBtn from '../src/components/tool/DomOSToolBtn.svelte';
...
describe('DomOSTool — action="click"', () => {
  it('registers the tool on mount with the correct name and description', () => {
    const { client } = mountWithClient(DomOSTool, {
      name: 'clear_cart',
  ...
  it('unregisters the tool on unmount', () => {
    const { client, instance } = mountWithClient(DomOSTool, {
      name: 'nav_tool',
      description: 'Nav',
      action: 'click',
      children: () => {},
...
describe('DomOSTool — runtime errors', () => {
  it('throws if both action and handler are provided', () => {
    domosClient.set(makeMockClient() as any);
    expect(() =>
      mount(DomOSTool, {
        target: container,
...
// ============================================================
// DomOSTool — update() re-registration on context change
// Tests agentTool.update() directly — this is what Svelte calls when
// the $derived fullDescription changes inside DomOSTool.svelte
// ============================================================

describe('DomOSTool — context reactivity (update)', () => {
...
describe('DomOSToolBtn', () => {
  it('registers the tool on mount', () => {
    const handler = vi.fn();
    const { client } = mountWithClient(DomOSToolBtn, {
      name: 'btn',
  ...
  it('unregisters on unmount', () => {
    const { client, instance } = mountWithClient(DomOSToolBtn, {
      name: 'btn',
      description: 'Btn',
      handler: vi.fn(),
      children: () => {},
