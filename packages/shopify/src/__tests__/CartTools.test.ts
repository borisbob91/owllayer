...
function getHandler(owllayer: ReturnType<typeof makeOwlLayerMock>, toolName: string) {
  const call = owllayer.registerTool.mock.calls.find(([name]: [string]) => name === toolName);
...
describe('registerCartTools', () => {
  ...
  it('enregistre exactement 4 tools', () => {
    const names = owllayer.registerTool.mock.calls.map(([name]: [string]) => name);
  ...
  it('add_to_cart a risk: low', () => {
    const call = owllayer.registerTool.mock.calls.find(([n]: [string]) => n === 'add_to_cart');
    expect(call?.[1].risk).toBe('low');
  });

  it('update_cart a risk: low', () => {
    const call = owllayer.registerTool.mock.calls.find(([n]: [string]) => n === 'update_cart');
    expect(call?.[1].risk).toBe('low');
  });

  it('remove_from_cart a risk: low', () => {
    const call = owllayer.registerTool.mock.calls.find(([n]: [string]) => n === 'remove_from_cart');
    expect(call?.[1].risk).toBe('low');
  });

  it('get_cart a risk: none', () => {
    const call = owllayer.registerTool.mock.calls.find(([n]: [string]) => n === 'get_cart');
  ...
  describe('add_to_cart handler', () => {
    it('ajoute un article et retourne le succès avec itemCount et cartTotal', async () => {
      ...
      const result = await getHandler(owllayer, 'add_to_cart')({ variantId: '111', qty: 1 });
      ...
      expect(owllayer.updateContext).toHaveBeenCalledTimes(1);
    });
    ...
    it('utilise qty=1 par défaut si absent', async () => {
      ...
      const result = await getHandler(owllayer, 'add_to_cart')({ variantId: '111' });
      expect(result.success).toBe(true);
    ...
    it('retourne { success: false } si variante out-of-stock (422)', async () => {
      ...
      const result = await getHandler(owllayer, 'add_to_cart')({ variantId: '999', qty: 1 });
      expect(result.success).toBe(false);
      expect(result.error).toBe('Ce produit est épuisé.');
      expect(owllayer.updateContext).not.toHaveBeenCalled();
    });
    ...
    it('utilise la locale-aware URL', async () => {
      ...
      await getHandler(owllayer, 'add_to_cart')({ variantId: '111', qty: 1 });
  ...
  describe('update_cart handler', () => {
    it('modifie la quantité et met à jour le contexte', async () => {
      ...
      const result = await getHandler(owllayer, 'update_cart')({ variantId: '111', qty: 2 });
      expect(result.success).toBe(true);
      expect(result.itemCount).toBe(1);
      expect(result.cartTotal).toBe('49.99 EUR');
      expect(owllayer.updateContext).toHaveBeenCalledTimes(1);
    ...
    it('envoie les bons headers et body', async () => {
      vi.stubGlobal('fetch', makeFetchOk(mockCart));
      await getHandler(owllayer, 'update_cart')({ variantId: '111', qty: 3 });
    ...
    it('retourne { success: false } sur erreur API', async () => {
      ...
      const result = await getHandler(owllayer, 'update_cart')({ variantId: '999', qty: 1 });
      expect(result.success).toBe(false);
    ...
    it('retourne { success: false } sur erreur réseau', async () => {
      vi.stubGlobal('fetch', vi.fn().mockRejectedValue(new Error('Network error')));
      const result = await getHandler(owllayer, 'update_cart')({ variantId: '111', qty: 1 });
      expect(result.success).toBe(false);
    });
  });
  ...
  describe('remove_from_cart handler', () => {
    it('supprime l\'article et retourne removedTitle + itemCount', async () => {
      ...
      const result = await getHandler(owllayer, 'remove_from_cart')({ variantId: '111' });
      expect(result.success).toBe(true);
  ...
  describe('get_cart handler', () => {
    it('retourne le contenu du panier au format OwlLayer', async () => {
      vi.stubGlobal('fetch', makeFetchOk(mockCart));
      const result = await getHandler(owllayer, 'get_cart')({});
      expect(result.success).toBe(true);
    ...
    it('n\'appelle pas updateContext (lecture seule)', async () => {
      vi.stubGlobal('fetch', makeFetchOk(mockCart));
      await getHandler(owllayer, 'get_cart')({});
      expect(owllayer.updateContext).not.toHaveBeenCalled();
    });
