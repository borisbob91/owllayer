...
import type { ApiClient, PromptEntry, SystemPromptConfig } from '../api.js';
...
export function PromptsPage({ api }: PromptsPageProps) {
  ...
  const handleDelete = async () => {
    if (!selectedKey || !confirm('Supprimer ce prompt ?')) return;
    try {
  ...
  return (
    <div>
      <h2 style={{ fontSize: 18, fontWeight: 700, color: TEXT, margin: '0 0 20px' }}>Prompts système</h2>
      ...
      {!selectedKey && (
        <p style={{ color: MUTED, fontSize: 13 }}>Sélectionnez une API key pour configurer son prompt.</p>
      )}
