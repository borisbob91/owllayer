import type { LLMToolCall, ToolDeclaration } from '@domos/core';
  ...
  async create(input: LiveKitAgentSessionFactoryInput): Promise<LiveKitAgentSessionLike> {
    ...
    const agentSession = new runtime.AgentSession({
      ...
      userData: {
        ...
        domosSessionId: input.session.sessionId,
      },
    });
...
export interface DomOSLiveKitAgentBridgeOptions {
  toolExecutor: DomOSToolExecutor;
  contextBridge?: DomOSContextBridge;
  roomManager?: LiveKitRoomManager;
  agentSessionFactory?: LiveKitAgentSessionFactory;
  responseTargetFactory?: (
    session: DomOSBridgeSessionSnapshot
  ) => DomOSToolResponseTarget | undefined;
  onEvent?: DomOSLiveKitBridgeEventListener;
}

export interface DomOSLiveKitAgentBridgeState {
  sessionId: string;
  context: DomOSContextSnapshot;
  room: LiveKitRoomHandle;
  agentSession: LiveKitAgentSessionLike;
...
export class DomOSLiveKitAgentBridge {
  ...
  async start(session: DomOSBridgeSessionSnapshot): Promise<DomOSLiveKitAgentBridgeState> {
    ...
    const state: DomOSLiveKitAgentBridgeState = {
      sessionId: session.sessionId,
      context,
      room,
      agentSession,
  ...
  async updateContext(session: DomOSBridgeSessionSnapshot): Promise<void> {
  ...
  getState(sessionId: string): DomOSLiveKitAgentBridgeState | undefined {
    return this.states.get(sessionId);
  }

  async close(sessionId: string, reason = 'domos_session_closed'): Promise<void> {
...
function buildAgentToolDescription(tool: ToolDeclaration): string {
  ...
  return `${tool.description}\nDomOS HITL risk level: ${tool.risk}. DomOS decides approval and execution.`;
}
