...
export interface OwlLayerBridgeSessionSnapshot {
  sessionId: string;
  apiKey?: string;
...
export interface OwlLayerContextBridgeOptions {
...
export interface OwlLayerContextSnapshot {
  sessionId: string;
  instructions: string;
  context: ShadowContext;
...
export class OwlLayerContextBridge {
  private readonly options: Required<OwlLayerContextBridgeOptions>;

  constructor(options: OwlLayerContextBridgeOptions = {}) {
    this.options = {
  ...
  buildSnapshot(input: OwlLayerBridgeSessionSnapshot): OwlLayerContextSnapshot {
  ...
  private buildInstructions(
    input: OwlLayerBridgeSessionSnapshot,
    context: ShadowContext,
    toolSummary: string
  ): string {
    ...
    const parts = [
      basePrompt.trim(),
      '[OWLLAYER_SHADOW_CONTEXT]',
      `Session: ${input.sessionId}`,
      `URL: ${context.url || '(unknown)'}`,
      `Title: ${context.title || '(none)'}`,
      `Data: ${contextJson}`,
      '',
      '[OWLLAYER_NEURAL_BINDING]',
      'The UI exposes only the tools that are currently mounted and effective in OwlLayer.',
      'A mounted component can add tools; an unmounted component can remove them.',
      'Client tools must be executed by OwlLayer through AITP in the browser runtime, never directly by LiveKit.',
      'Server tools and HITL approvals stay under OwlLayer server policy.',
      '',
      '[OWLLAYER_EFFECTIVE_TOOLS]',
      toolSummary || 'No effective OwlLayer tools are currently available.',
...
function compactValue(
  value: unknown,
  options: Required<OwlLayerContextBridgeOptions>,
  depth: number
): unknown {
