...
import { OwlLayerAgent } from '../src/agent/OwlLayerAgent.js';
...
describe('OwlLayerAgent', () => {
  it('hydrate un snapshot existant', async () => {
    ...
    const agent = new OwlLayerAgent({ adapter });
  ...
  it('met a jour la memoire et flush apres reponse', async () => {
    const adapter = new TestAdapter();
    const agent = new OwlLayerAgent({ adapter, saveDebounceMs: 5 });
  ...
  it('debounce les sauvegardes', async () => {
    vi.useFakeTimers();
    const adapter = new TestAdapter();
    const agent = new OwlLayerAgent({ adapter, saveDebounceMs: 100 });
