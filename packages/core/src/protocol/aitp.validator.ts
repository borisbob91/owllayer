...
import type { ToolParameterProperty } from './aitp.types.js';

// ============================================================
// Schemas Zod pour valider chaque type de message AITP
// ============================================================

const aitpToolTypeSchema = z.enum(['STRING', 'NUMBER', 'BOOLEAN', 'OBJECT', 'ARRAY']);

const toolParameterPropertySchema: z.ZodType<ToolParameterProperty> = z.lazy(() => z.object({
  type: aitpToolTypeSchema,
...
/**
 * Schema de base d'un message AITP (sans validation du payload).
 */
...
/**
 * Valider un message AITP complet (enveloppe + payload).
 */
export function validateMessage(raw: unknown): ValidationResult {
  ...
  if (!baseResult.success) {
    return {
      success: false,
      error: `Invalid AITP message: ${baseResult.error.issues[0]?.message}`,
    };
  }
