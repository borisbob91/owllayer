<?php
/**
 * DomOS_REST_API — Endpoints REST WordPress pour Store Connect (Sprint 7)
 *
 * Endpoints exposés :
 *   GET  /wp-json/domos/v1/health   — Vérification plugin par DomOS Cloud (CU-SC02 étape 10)
 *   POST /wp-json/domos/v1/connect  — Réception clé API après wc-auth (CU-SC02 étape 12-15)
 *   POST /wp-json/domos/v1/webhook  — Réception webhooks DomOS Cloud (order.created, etc.)
 *
 * Sécurité :
 *   - Toutes les routes sont protégées par verify_domos_signature() (HMAC-SHA256)
...
class DomOS_REST_API {
    ...
    /**
     * Enregistre les routes REST DomOS.
     ...
     * Appelé par DomOS Cloud pour vérifier que le plugin est actif (CU-SC02 étape 10).
     */
     ...
     *
     * Reçoit la clé API DomOS Cloud après validation wc-auth.
     * Payload attendu : { "api_key": "pk_live_woo_...", "shop_id": "uuid" }
     *
     * Stocke api_key + shop_id dans domos_woo_settings (wp_options).
     */
    public function handle_connect( WP_REST_Request $request ): WP_REST_Response {
        ...
        // Validation stricte du format clé API DomOS (pk_live_woo_ ou pk_dev_woo_)
        ...
        // Stocker la clé API + shop_id reçus depuis DomOS Cloud
     ...
     *
     * Reçoit les webhooks DomOS Cloud (order.created, order.updated, etc.).
     * Déclenche le hook WordPress 'domos_webhook_received' pour extensibilité.
    ...
    public function handle_webhook( WP_REST_Request $request ): WP_REST_Response {
        ...
        /**
         * Fires when a DomOS Cloud webhook is received.
         *
    ...
    /**
     * Vérifie la signature HMAC-SHA256 de toutes les requêtes DomOS Cloud.
     *
     * Header attendu : X-DomOS-Signature: sha256={hmac}
     * Secret partagé : domos_woo_settings['webhook_secret']
     *
     ...
     */
    public function verify_domos_signature( WP_REST_Request $request ): bool|WP_Error {
        $signature = $request->get_header( 'X-DomOS-Signature' );
        if ( empty( $signature ) ) {
            return new WP_Error( 'rest_forbidden', 'Missing X-DomOS-Signature header', [ 'status' => 403 ] );
        }
