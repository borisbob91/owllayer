...
//   • Registers a tool via the OwlLayer client (onMounted / onUnmounted)
...
if (props.action && props.handler) {
  throw new Error(`OwlLayerTool "${props.name}": provide action OR handler, not both.`);
}
if (!props.action && !props.handler) {
  throw new Error(`OwlLayerTool "${props.name}": action or handler is required.`);
}
...
if (!client) {
  throw new Error('OwlLayerTool: OwlLayerPlugin is not installed.');
}
