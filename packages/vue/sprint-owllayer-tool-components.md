# Sprint: `OwlLayerTool` + `OwlLayerToolBtn` — @owllayer/vue
...
**Idiome Vue :** composants SFC avec `<script setup>`, `useAgentTool` en interne, `<slot>` pour les enfants. Pas de directive (`v-owllayer-tool`) pour cette itération — composant SFC suffisant et plus simple à tester.
...
**Solution dans `OwlLayerTool.vue` :** utiliser `watch` sur `fullDescription` (computed) pour re-enregistrer manuellement via le client injecté :
...
| `src/components/tool/OwlLayerTool.vue` | CRÉER |
| `src/components/tool/OwlLayerToolBtn.vue` | CRÉER |
...
## Phase 2 — `OwlLayerTool.vue`

### `src/components/tool/OwlLayerTool.vue`
...
if (props.action && props.handler) {
  throw new Error(`OwlLayerTool "${props.name}": utilisez action OU handler, pas les deux.`);
}
if (!props.action && !props.handler) {
  throw new Error(`OwlLayerTool "${props.name}": action ou handler requis.`);
}
...
if (!client) {
  throw new Error('OwlLayerTool: OwlLayerPlugin non installé.');
}
...
## Phase 3 — `OwlLayerToolBtn.vue`

### `src/components/tool/OwlLayerToolBtn.vue`
...
if (!client) {
  throw new Error('OwlLayerToolBtn: OwlLayerPlugin non installé.');
}
...
export type { OwlLayerToolProps, OwlLayerToolBtnProps } from './components/tool/types.js';
```
...
```bash
pnpm --filter @owllayer/vue build       # exit 0, pas d'erreurs TS
pnpm --filter demo-vue dev           # OwlLayerToolBtn "Vider" cliquable agent + humain
```
...
<template>
  ...
  <!-- ✅ OwlLayerTool + action="click" — lien RouterLink -->
  <OwlLayerTool
    name="start_checkout"
