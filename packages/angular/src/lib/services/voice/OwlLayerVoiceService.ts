// ============================================================
// OwlLayerVoiceService — Service Angular centralisé pour l'audio voice.
//
// Responsabilités :
...
@Injectable()
export class OwlLayerVoiceService implements OnDestroy {
  ...
  constructor() {
    ...
    this.unsubAudioOutput = this.owllayer.onAudioOutput((audioBase64, mimeType) => {
      this.playChunk(audioBase64, mimeType);
    });

    // Abonnement aux événements d'état vocal du serveur
    this.owllayer.client.on({
  ...
  async startCapture(): Promise<void> {
    ...
    if (this.isPlaybackActive() || this.owllayer.state() === 'speaking') {
      if (this.voiceMachine.state === 'playing') {
        this.voiceMachine.dispatch('BARGE_IN');
      }
      this.owllayer.sendInterrupt();
      this.stopPlaybackInternal();
    }
    ...
    this.processor.onaudioprocess = (event) => {
      ...
      // Encodage PCM16 base64 via @owllayer/audio
      ...
      if (!this.isMuted()) {
        this.owllayer.sendAudioStream(audioBase64, 'audio/pcm;rate=16000');
      }
    };
  ...
  stopCapture(reason: Parameters<typeof this.owllayer.sendAudioEnd>[0] = 'user_stop', notifyServer = true): void {
    if (notifyServer && this.isRecording()) {
      this.owllayer.sendAudioEnd(reason);
    }
