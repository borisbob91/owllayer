/**
 * Utilitaire de rendu Markdown léger et sécurisé (anti-XSS) pour les messages du chat.
 */
...
export function renderMarkdown(rawText: string): string {
  ...
  // Ex: "Voici les infos : * **Nom :** ... * **Prix :** ..." -> saut de ligne avant chaque puce
  text = text.replace(/([^\n])\s+\*\s+(\*\*|\w)/g, '$1\n* $2');
  text = text.replace(/([^\n])\s+-\s+(\*\*|\w)/g, '$1\n- $2');
  ...
  // 7. Liens [texte](url)
  text = text.replace(/\[([^\]]+)\]\((https?:\/\/[^\s\)]+)\)/g, '<a href="$2" target="_blank" rel="noopener noreferrer" class="text-emerald-400 underline hover:text-emerald-300">$1</a>');
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (listMatch) {
      ...
      result.push(`<li class="flex items-start gap-1.5"><span class="text-emerald-400 select-none leading-relaxed">•</span><span>${listMatch[1]}</span></li>`);
    } else {
      if (inList) {
        result.push('</ul>');
        inList = false;
