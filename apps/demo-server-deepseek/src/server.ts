...
// Charger le .env de apps/demo-server-deepseek/ de manière absolue et robuste (racine ou dossier local)
configDotenv({ path: join(__dirname, "../.env") });
configDotenv({ path: join(process.cwd(), "apps/demo-server-deepseek/.env") });
configDotenv();
...
const DEEPSEEK_TIMEOUT_MS = Number.parseInt(
...
const OWLLAYER_TOOL_TIMEOUT_MS = Number.parseInt(
  process.env.OWLLAYER_TOOL_TIMEOUT_MS || "60000",
  10,
);

const OWLLAYER_API_KEY = process.env.OWLLAYER_API_KEY || "pk_demo_local";
const OWLLAYER_ADMIN_API_KEY =
  process.env.OWLLAYER_ADMIN_API_KEY || "pk_78ab37_vue_admin";
const OWLLAYER_TRAVEL_API_KEY =
  process.env.OWLLAYER_TRAVEL_API_KEY ||
  process.env.OWLLAYER_HOME_API_KEY ||
  "pk_78ab37_svelte_travel";
const OWLLAYER_ANGULAR_API_KEY =
  process.env.OWLLAYER_ANGULAR_API_KEY || "pk_78ab37_angular_marketplace";
const OWLLAYER_BROWSER_API_KEY =
  process.env.OWLLAYER_BROWSER_API_KEY || "pk_browser_demo";
...
const ENABLE_VIRTUAL_LINES =
  process.env.OWLLAYER_ENABLE_VIRTUAL_LINES === "true";
...
if (!DEEPSEEK_API_KEY || DEEPSEEK_API_KEY === "your_deepseek_api_key_here") {
  log.warn(
    "Missing DEEPSEEK_API_KEY! Add your key in apps/demo-server-deepseek/.env",
  );
}

// ============================================================
// LLM Adapter (DeepSeek via OpenAI-compatible API)
// ============================================================
...
// DeepSeek is OpenAI-compatible for chat/completion, but not a native live audio provider in this repo.
...
const server = new OwlLayerServer({
  ...
  server: httpServer,
  path: "/owllayer",
  toolTimeout: OWLLAYER_TOOL_TIMEOUT_MS,
  ...
  virtualLines:
    ENABLE_VIRTUAL_LINES && OWLLAYER_API_KEY
      ? {
          lines: [
            {
...
if (REQUIRE_API_KEY && OWLLAYER_API_KEY) {
  ...
  log.warn(
    "Missing OWLLAYER_API_KEY! Connections will be rejected (requireAuth=true)",
  );
} else {
  log.info(
    "Client authentication without API key active (OWLLAYER_REQUIRE_API_KEY=false).",
  );
}
...
if (OWLLAYER_ADMIN_API_KEY) {
  ...
  log.info(
    `Admin API key registered with dedicated prompt (${DEFAULT_LANGUAGE}): ${OWLLAYER_ADMIN_API_KEY.slice(
      0,
      12,
    )}...`,
  );
...
if (OWLLAYER_TRAVEL_API_KEY) {
  ...
  log.info(
    `Travel (Svelte) API key registered with dedicated prompt (${DEFAULT_LANGUAGE}): ${OWLLAYER_TRAVEL_API_KEY.slice(
      0,
      12,
    )}...`,
  );
...
if (OWLLAYER_ANGULAR_API_KEY) {
  ...
  log.info(
    `Angular Marketplace API key registered: ${OWLLAYER_ANGULAR_API_KEY.slice(
      0,
      12,
    )}...`,
  );
...
if (OWLLAYER_BROWSER_API_KEY) {
  ...
  log.info(
    `Vanilla Browser API key registered: ${OWLLAYER_BROWSER_API_KEY.slice(
      0,
      12,
    )}...`,
  );
...
server.tool("get_server_time", async () => {
  return {
    ...
    formatted: new Date().toLocaleString(
      DEFAULT_LANGUAGE === "fr" ? "fr-FR" : "en-US",
      {
