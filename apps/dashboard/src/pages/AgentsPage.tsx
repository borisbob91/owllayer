...
import { api, type PromptEntry, type ApiKeyEntry, type SystemPromptConfig, type SystemPromptValue } from '../api';
...
export default function AgentsPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setConfig({ ...EMPTY_CONFIG });
      setMessage({ type: 'success', text: 'Agent supprimé — le prompt du code sera utilisé' });
      await fetchAll();
    } catch (err) {
      setMessage({ type: 'error', text: err instanceof Error ? err.message : String(err) });
    }
  ...
  return (
    <div>
      ...
      <p className="text-sm text-gray-400 mb-6">
        Configurer le system prompt de chaque agent (API key). Les agents sans prompt utilisent le prompt par défaut du code.
      </p>

      {/* Résumé */}
      <div className="mb-6 flex gap-3">
        <div className="bg-gray-800 rounded-lg px-4 py-3 flex-1 text-center">
          <div className="text-2xl font-bold text-white">{allKeys.length}</div>
          <div className="text-xs text-gray-400 mt-0.5">API Keys</div>
        </div>
        <div className="bg-gray-800 rounded-lg px-4 py-3 flex-1 text-center">
          <div className="text-2xl font-bold text-green-400">{prompts.length}</div>
          <div className="text-xs text-gray-400 mt-0.5">Agents configurés</div>
        </div>
        <div className="bg-gray-800 rounded-lg px-4 py-3 flex-1 text-center">
          <div className="text-2xl font-bold text-yellow-400">{allKeys.length - assignedKeys.size}</div>
          <div className="text-xs text-gray-400 mt-0.5">Sans agent</div>
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        >
          ...
          {allKeys.map(entry => {
            ...
            const label = entry.name ? `${entry.name} (${entry.masked})` : entry.masked;
            return (
              <option key={entry.key} value={entry.key}>
                {label} {hasAgent ? '✓ agent configuré' : '⚠ sans agent'}
              </option>
            );
          })}
        </select>
        {allKeys.length === 0 && (
          <p className="mt-2 text-sm text-yellow-500">
            Aucune API key enregistrée. Ajoutez des clés dans l'onglet <strong>API Keys</strong> d'abord.
          </p>
        )}
      </div>
      ...
      {selectedKey && (
        <div className="space-y-4">
          ...
          {/* Personnalité */}
          <Field label="Personnalité" optional>
            <textarea
          ...
          {/* Capacités */}
          <Field label="Capacités" optional>
          ...
          <Field label="Instructions tools" optional>
            <textarea
              ...
              placeholder="Instructions spécifiques pour l'usage des tools..."
              rows={2}
              className="input-field"
            />
          </Field>
          ...
          {/* Format de réponse */}
          <Field label="Format de réponse" optional>
            <textarea
              value={config.responseFormat || ''}
              onChange={e => setConfig(c => ({ ...c, responseFormat: e.target.value }))}
              placeholder="Réponds en français, de manière concise..."
              rows={2}
              className="input-field"
            />
          </Field>
          ...
          <div className="flex items-center gap-3 pt-4 border-t border-gray-700">
            ...
            <span className="text-xs text-gray-500 ml-auto">
              {hasOverride ? 'Agent actif — remplace le prompt du code' : 'Pas d\'agent — le prompt du code est utilisé'}
            </span>
          </div>
        </div>
      )}
