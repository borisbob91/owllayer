...
description: Protéger les actions sensibles demandées par l'agent avec la politique d'approbation OwlLayer.
---
...
OwlLayer est un SDK d'interfaces agentiques : l'agent n'agit que par les tools déclarés par l'application. **HITL** (*Human-in-the-Loop*) ajoute une décision humaine lorsque l'impact du tool exige une confirmation.
...
`OwlLayerServer` expose actuellement le blocage public d'un tool. Si votre politique de production doit changer dynamiquement, recréez une configuration de serveur cohérente avec la politique souhaitée.
