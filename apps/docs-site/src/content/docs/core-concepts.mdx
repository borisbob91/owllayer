...
DomOS est un framework d'**Agentic UI**. Il ne remplace pas votre interface et ne genere pas une nouvelle UI par-dessus votre produit. Il rend votre interface existante pilotable par un agent IA, avec des actions explicites, un contexte observable et des garde-fous.

## Vue d'ensemble

<CardGrid>
  <Card title="Agentic UI" icon="rocket">
    Une interface produit qui expose volontairement certaines actions a un agent IA, sans lui donner un acces libre au DOM.
  </Card>
  <Card title="DomOSClient" icon="setting">
    Le runtime front commun de `@domos/core`, partage par les SDK React, Vue, Svelte, Angular et Browser.
  </Card>
...
Une Agentic UI est une interface que l'agent peut piloter par des points d'entree explicites.

L'agent ne manipule pas librement le DOM. Il ne devine pas les clics a effectuer. Il recoit un contexte structure et une liste de tools declares par l'application, puis il agit uniquement a travers ces tools.
...
Dans DomOS, l'application reste proprietaire de sa logique metier. L'agent appelle seulement les actions que le produit accepte d'exposer.
...
Le point a retenir : Angular change l'environnement asynchrone autour du handler, pas le protocole ni le contrat de retour.
