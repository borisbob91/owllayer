---
title: "OwlLayer Widget - interface conversationnelle injectable"
description: Ajouter une interface de conversation OwlLayer texte ou vocale dans un frontend existant.
---

import { CardGrid, LinkCard } from '@astrojs/starlight/components';

# OwlLayer Widget - interface conversationnelle injectable

OwlLayer permet à un agent IA d'agir dans une interface existante par des tools que le développeur déclare explicitement. Le `OwlLayerWidget` est la surface conversationnelle prête à l'emploi pour dialoguer avec cet agent ; il ne remplace ni vos tools, ni votre contexte métier, ni vos règles HITL.

Le widget est disponible dans les SDK React, Vue, Svelte et Angular. Pour une intégration sans framework, consultez le [SDK Browser](/browser/widget-voice-session/).

![Widget OwlLayer : panneau conversationnel avec modes audio et texte dans une interface web.](../../assets/docs/widget-modes.png)
...
# Angular
pnpm add @owllayer/angular @owllayer/core
```
...
Le SDK Angular expose `OwlLayerWidgetComponent` comme composant standalone. Son montage, son intégration HITL et ses paramètres sont documentés dans le guide [Widget Angular](/angular/widget/).
...
- Angular : `packages/angular/src/lib/components/widget/OwlLayerWidgetComponent.ts`
...
<CardGrid>
  <LinkCard
    title="Bien debuter"
    description="Ajouter le widget dans une premiere application."
    href="/getting-started/"
  />
  <LinkCard
    title="Pipeline audio"
    description="Respecter les regles de lecture PCM du mode vocal."
    href="/audio_pipeline_rules/"
  />
  <LinkCard
    title="SDK React"
    description="Voir l'integration widget cote React."
    href="/react/widget/"
  />
  <LinkCard
    title="SDK Angular"
    description="Monter le composant widget standalone Angular."
    href="/angular/widget/"
  />
</CardGrid>
