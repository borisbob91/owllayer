...
description: Les messages du protocole AITP réellement échangés entre OwlLayerClient et OwlLayerServer.
---

import { Card, CardGrid, LinkCard } from '@astrojs/starlight/components';

OwlLayer est un SDK d'**AI-driven interfaces**, ou interfaces agentiques : l'agent agit seulement par les actions déclarées par l'application.

**AITP** signifie **Agent-to-Interface Transfer Protocol**. C'est le protocole d'échange de OwlLayer : il transporte le contexte utile et les tools que l'application choisit d'exposer, puis les demandes d'action et leurs résultats.

AITP ne donne pas à l'agent un accès général au DOM. Il transporte des messages structurés entre :

- `OwlLayerClient`, partagé par les SDK frontend ;
- `OwlLayerServer`, qui maintient la session et appelle l'adapter LLM.
...
Dans `@owllayer/core`, chaque message porte un identifiant et un timestamp au niveau racine :
...
Le champ `meta` est optionnel et peut transporter un `sessionId` ou un `token`. Les payloads sont validés dans `@owllayer/core` avant d'être traités.
...
![Cycle AITP d'un tool : contexte, tool call local, tool result et réponse agent.](../../assets/docs/aitp-tool-lifecycle.png)

## Approbation humaine

Lorsque le flux nécessite une décision de l'utilisateur, AITP transporte :
...
AITP couvre également les échanges vocaux :
...
| Export | Utilité |
| --- | --- |
| `AITP_VERSION` | Version de protocole attendue (`1.0.0`). |
| `MessageType` | Enumération des messages AITP. |
