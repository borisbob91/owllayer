...
DomOS est un SDK d'interfaces pilotées par l'IA. Il permet à un agent d'agir dans une interface existante par les actions que votre application déclare explicitement.

Dans ce guide, l'interface fournit son contexte utile et ses outils autorisés ; le serveur dialogue avec le modèle ; **ADTP** (*Agent-to-DOM Transfer Protocol*) transporte ces échanges sur WebSocket.
