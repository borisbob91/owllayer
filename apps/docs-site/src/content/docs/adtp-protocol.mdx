...
**ADTP** signifie **Agent-to-DOM Transfer Protocol**. C'est le protocole d'échange de DomOS : il transporte le contexte utile et les outils que l'application choisit d'exposer, puis les demandes d'action et leurs résultats.

ADTP ne donne pas à l'agent un accès général au DOM. Il transporte des messages structurés entre :

- `DomOSClient`, partagé par les SDKs frontend ;
