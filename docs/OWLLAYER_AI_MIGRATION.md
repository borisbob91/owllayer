...
produit, mais **ADTP reste le nom de compatibilité dès qu'il s'agit du code,
...
La page de référence du protocole est [ADTP_PROTOCOL.md](./ADTP_PROTOCOL.md) et
...
actuel doivent continuer à importer depuis `@domos/*`. Pour le protocole,
`@domos/core` exporte actuellement `ADTP_VERSION`, `MessageType`, `Messages`,
`encode`, `decode`, `tryDecode`, `validateMessage` et les types `ADTP*`.
...
| --- | --- | --- |
| `@domos/core` | `@owllayer/core` | Garder `@domos/core` dans le code. |
| `@domos/ui` | `@owllayer/ui` | Garder `@domos/ui` dans le code. |
| `@domos/browser` | `@owllayer/browser` | Garder `@domos/browser` dans le code. |
| `@domos/react` | `@owllayer/react` | Garder `@domos/react` dans le code. |
| `@domos/vue` | `@owllayer/vue` | Garder `@domos/vue` dans le code. |
| `@domos/svelte` | `@owllayer/svelte` | Garder `@domos/svelte` dans le code. |
| `@domos/angular` | `@owllayer/angular` | Garder `@domos/angular` dans le code. |
| `@domos/server` | `@owllayer/server` | Garder `@domos/server` dans le code. |
| `@domos/adapter-openai` | `@owllayer/adapter-openai` | Garder l'import actuel. |
| `@domos/adapter-google` | `@owllayer/adapter-google` | Garder l'import actuel. |
| `@domos/adapter-anthropic` | `@owllayer/adapter-anthropic` | Garder l'import actuel. |
| `@domos/adapter-livekit` | `@owllayer/adapter-livekit` | Garder l'import actuel. |
...
| --- | --- | --- |
| 0. Documentation | **Issue #15** | AITP est le terme public ; ADTP, `@domos/*` et les exports actuels restent les références exécutables. |
...
| --- | --- | --- |
| `docs/ADTP_PROTOCOL.md` | Conserver le fichier pendant la compatibilité ; page publique cible `/aitp-protocol/` | Contenu aligné sur AITP, sans renommage de fichier. |
| `/adtp-protocol/` | `/aitp-protocol/` | Prévoir une redirection permanente ou un alias compatible dans une issue `docs-site/` distincte ; conserver l'ancien lien pendant la période de dépréciation. |
...
- vérifier que toute première mention du protocole distingue AITP du wire ADTP
  courant ;
...
- vérifier que les imports et identifiants dans les blocs de code existent
  encore dans les packages `@domos/*` ;
- vérifier les liens relatifs vers `ADTP_PROTOCOL.md`, `CONCEPTS.md`,
...
Pour le suivi de cette livraison, consulter [l'issue #15](https://github.com/borisbob91/domos/issues/15).
